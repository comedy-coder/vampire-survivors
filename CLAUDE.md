...
| `scripts/gem.gd` | XP gem with magnet-pull behavior (value 1 normal, 5 from elites). |
| `scripts/pickup.gd` | Floor drops: `heal` / `magnet` / `bomb`. |
...
- **Time & spawning** (`_process`): a single `time` accumulator drives everything.
  Spawn interval and enemy HP/speed scale with `time`. Enemy variety unlocks over time
  (basic → runner → ranger → tank robot) in `_spawn_enemy`.
...
- **Artifacts** (`ARTIFACTS`): one-time passives (magnet, phoenix revive, spirit
  familiar, regen, crit, double-XP). The familiar (`scripts/familiar.gd`) is an
  autonomous pet spawned via `_spawn_familiar` that orbits the player and auto-fires.
...
- **Chests** (`_spawn_chest`/`_open_chest`): dropped by bosses; grant 1–3 random pool
  upgrades.
- **Meta-progression** (`META_UPGRADES`, `_*_meta*`, shop): on death, gold =
  `kills + boss_count*20 + time/5`. Spent in the character-select shop on permanent
  upgrades, applied to the player at character pick via `_apply_meta_upgrades`.
